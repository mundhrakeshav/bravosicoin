...
contract CDPEngine is Auth, Pausable {
    ...
    uint256 public totalUnbackedDai;
    ...
    function grab(
        ...
        int256 deltaCoin = Math.mul(col.rateAcc, deltaDebt);
        gem[collateralType][gemDst] = Math.sub(gem[collateralType][gemDst], deltaCol);

        unbackedDebt[debtDst] = Math.sub(unbackedDebt[debtDst], deltaCoin);

        totalUnbackedDai = Math.sub(totalUnbackedDai, deltaCoin);
    }
    ...
    //
    function updateRateAcc(bytes32 _collateralType, address _coinDst, int256 _deltaRate) external auth notPaused {
    ...
    function burn(uint256 rad) external {
        unbackedDebt[msg.sender] -= rad;
        balance[msg.sender] -= rad;

        totalUnbackedDai -= rad;
        systemDebt -= rad;
    }

    function mint(address debtDst, address coinDst, uint256 rad) external auth notPaused {
